...
static bool32 AccuracyCalcHelper(u16 move)
{
	...
	if (WEATHER_HAS_EFFECT && (gBattleWeather & WEATHER_HAIL_ANY) && (gBattleMoves[move].effect == EFFECT_BLIZZARD))
	{
		JumpIfMoveFailed(7, move);
		return TRUE;
	}
...
static void atk42_setroost(void)
{
    ...
    // Pure flying type.
    if (gBattleMons[gBattlerAttacker].type1 == TYPE_WIND && gBattleMons[gBattlerAttacker].type2 == TYPE_WIND)
    {
        gBattleStruct->roostTypes[gBattlerAttacker][0] = TYPE_WIND;
        gBattleStruct->roostTypes[gBattlerAttacker][1] = TYPE_WIND;
        gBattleStruct->roostTypes[gBattlerAttacker][2] = TYPE_WIND;
        SET_BATTLER_TYPE(gBattlerAttacker, TYPE_NORMAL);
    }
    // Dual Type with Flying Type.
    else if ((gBattleMons[gBattlerAttacker].type1 == TYPE_WIND && gBattleMons[gBattlerAttacker].type2 != TYPE_WIND)
           ||(gBattleMons[gBattlerAttacker].type2 == TYPE_WIND && gBattleMons[gBattlerAttacker].type1 != TYPE_WIND))
    {
    ...
    // Non-flying type.
    else if (!IS_BATTLER_OF_TYPE(gBattlerAttacker, TYPE_WIND))
    {
        gBattleStruct->roostTypes[gBattlerAttacker][0] = gBattleMons[gBattlerAttacker].type1;
        gBattleStruct->roostTypes[gBattlerAttacker][1] = gBattleMons[gBattlerAttacker].type2;
    }
    ...
    else
    {
        ...
        if (gLastUsedItem > ITEM_SAFARI_BALL)
        {
            ...
            case ITEM_NET_BALL:
                ...
                break;
            case ITEM_DIVE_BALL:
                if ((GetCurrentMapType() == MAP_TYPE_UNDERWATER) || IS_BATTLER_OF_TYPE(gBattlerTarget, TYPE_WATER))
                    ballMultiplier = 35;
                else
                    ballMultiplier = 10;
                break;
