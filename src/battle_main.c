                                                                        ...
                                                                        | BATTLE_TYPE_TRAINER_HILL)))
    {
        ...
        for (i = 0; i < monsCount; i++)
        {
            ...
            case F_TRAINER_PARTY_FULL_CONTROL:
            {
                ...
                do
                {
                    CreateMonWithGenderNatureLetter(&party[i], partyData[i].species, partyData[i].lvl, fixedIV, partyData[i].gender, partyData[i].nature, 0);
                } while (IsMonShiny(&party[i]));
...
u8 IsRunningFromBattleImpossible(void)
{
    ...
    for (i = 0; i < gBattlersCount; i++)
    {
        ...
        if (side != GetBattlerSide(i)
         ...
         && !IS_BATTLER_OF_TYPE(gActiveBattler, TYPE_WIND)
...
s8 GetMovePriority(u32 battlerId, u16 move)
{
    ...
    if (GetBattlerAbility(battlerId) == ABILITY_GALE_WINGS
        && gBattleMoves[move].type == TYPE_WIND
    ...
    if (gBattleMoves[move].effect == EFFECT_WEATHER_BALL)
    {
        if (WEATHER_HAS_EFFECT)
        {
            if (gBattleWeather & WEATHER_RAIN_ANY)
                ...
                *(&gBattleStruct->dynamicMoveType) = TYPE_EARTH | 0x80;
        ...
        || gStatuses3[battlerAtk] & STATUS3_ELECTRIFIED)
    {
        gBattleStruct->dynamicMoveType = 0x80 | TYPE_ELECTRIC;
             ...
             && ((attackerAbility == ABILITY_PIXILATE && (ateType = TYPE_HEART))
                 ...
                 || (attackerAbility == ABILITY_AERILATE && (ateType = TYPE_WIND))
